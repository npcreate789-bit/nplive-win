...
class HookModePipeline:
    ...
    def _build_video_filter(
        ...
        2. **Rear-facing default** (``hook_encode_rear_facing``): no
           ``hflip``; ``transpose=1`` then ``vflip`` so the MP4 matches
           TikTok's rear-camera buffer (inject is back-only since v1.8.8).
    ...
    def encode_playlist(
        ...
        (``hook_encode_rear_facing``) we run ``transpose=1`` then
        ``vflip`` so the file matches the rear buffer without asking
        the customer to dial rotation on the PC. Set
        ``hook_encode_rear_facing`` to false for the older front-camera
        ``hflip`` + ``transpose=1`` chain.
